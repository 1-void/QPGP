...
fn normalize_oqs_name(name: &CStr) -> Option<CString> {
    let original = name.to_string_lossy();
    if !original.contains('-') {
        return None;
    }
    let normalized = original.to_lowercase().replace('-', "");
...
impl EvpPkeyCtx {
    ...
    pub fn new(ctx: &OsslContext, name: &CStr) -> Result<EvpPkeyCtx, Error> {
        ...
        if ptr.is_null() {
            if let Some(alt) = normalize_oqs_name(name) {
                let ptr = unsafe {
                    EVP_PKEY_CTX_new_from_name(
                        ctx.ptr(),
                        alt.as_ptr(),
...
mod tests {
    ...
    #[cfg(feature = "rfc9580")]
    #[test]
    fn import_rsa_dpq() -> Result<(), crate::Error> {
        // The inputs.
        ...
        let (e, data) = crate::pkey::RsaData::from_dpq(d, p, q)?;
